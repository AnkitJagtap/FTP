...
# Workshop material - Day #1 - AWS CodeCommit/Visual Studio (VS) Code
  ...
  * Clone repository at Gitbash
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/ftp71 `
    * `cd ftp71`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\ftp71
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE ftp71;` and click the lightning button
  * `CREATE USER 'ftp71'@'localhost' IDENTIFIED BY 'ftp71';`
  * `GRANT ALL ON ftp71.* TO 'ftp71'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp71
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.ftp71.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp71-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp71.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp71/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp71/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp71-0.0.1-SNAPSHOT.war ./ftp71.war`
   * `cd dist`
   * `jar -uvf ../ftp71.war *`
   * `cd ..`
   * `jar -tvf ftp71.war`
   * `cp ftp71.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp71.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp71/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "ftp71"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "ftp71 Sprint 1" as label 
